/**
 * Student Management E2E Tests - Real User Flow
 * 
...
test.describe('Student Management - Real User Flow', () => {
  ...
  test('displays student management interface correctly', async ({ page }) => {
    // Verify specific page heading with emoji
    await expect(page.getByRole('heading', { name: '📚 図書委員管理' })).toBeVisible()
  ...
  test('basic page elements load correctly', async ({ page }) => {
    ...
    console.log('✅ Student management page basic structure verified')
  })
  ...
  test('handles page navigation correctly', async ({ page }) => {
    ...
    // Verify page heading with emoji
    await expect(page.getByRole('heading', { name: '📚 図書委員管理' })).toBeVisible()
    
    console.log('✅ Student management page navigation verified')
  })
})