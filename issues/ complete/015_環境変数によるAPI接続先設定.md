# Issue #015: 環境変数によるAPI接続先設定とテスト環境管理
...
1. フロントエンドの環境変数対応
   ...
   - [x] `next.config.js`の確認（環境変数の公開設定）

2. バックエンドのポート設定対応
   - [x] `app.py`の修正

3. スクリプトの作成
   - [x] `scripts/dev.sh`の作成
   - [x] `scripts/test-env.sh`の作成
   - [x] `scripts/stop-all-tests.sh`の作成
   - [x] スクリプトに実行権限を付与

4. ドキュメントの更新
   - [x] `CLAUDE.md`の更新
   - [x] `docs/system_design.md`の更新
   - [ ] `docs/test_strategy.md`の更新（省略）

5. テスト
   - [x] 開発環境起動スクリプトの動作確認
   - [x] テスト環境起動スクリプトの動作確認（複数Issue同時起動）
   - [x] 環境変数による接続先切り替えの確認
...
### 実装内容の詳細
1. **フロントエンド環境変数対応**
   - `api.ts`でprocess.env.NEXT_PUBLIC_API_BASE_URLから接続先を取得
   - デフォルト値として`http://localhost:5200/api`を設定
   - `.env.local.example`ファイルを作成し、サンプル設定を提供

2. **バックエンド環境変数対応**  
   - FLASK_RUN_PORTとFLASK_PORT両方をサポート（互換性のため）
   - デフォルトポート5200を維持

3. **起動スクリプト**
   - `scripts/dev.sh`: 開発環境一括起動
   - `scripts/test-env.sh`: Issue番号ベースのテスト環境管理（start/stop/status/restart）
   - `scripts/stop-all-tests.sh`: 全テスト環境一括停止
   - ログファイルはlogs/ディレクトリに保存

4. **ドキュメント更新**
   - CLAUDE.mdに環境変数とスクリプトの使用方法を追記
   - system_design.mdに環境設定セクションを追加

### テスト結果
- 開発環境起動スクリプトが正常動作
- Issue #15用のテスト環境（ポート3015/5015）が正常起動・停止
- 環境変数によるAPI接続先切り替えが正常動作